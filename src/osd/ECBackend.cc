  ...
  for(auto i = op.to_read.begin();
    ...
    for (auto j = i->second.begin(); j != i->second.end(); ++j) {
      ...
      if ((op.subchunks.find(i->first)->second.size() == 1) && 
        ...
        dout(0)<<"ErasureCodeMyClay: handle_sub_read() COMPLETE store->read(oid,offset,len): "<<ghobject_t(i->first, ghobject_t::NO_GEN, shard)<<", "<<(j->get<0>())<<", "<<(j->get<1>())<<dendl;
        r = store->read(
	  ch,
    ...
    dout(0)<<"ErasureCodeMyClay: handle_sub_read() COMPLETE read done" << dendl;
      } else {
        dout(25) << __func__ << " case2: going to do fragmented read." << dendl;
        int subchunk_size =
          sinfo.get_chunk_size() / ec_impl->get_sub_chunk_count();
        bool error = false;
        dout(0)<<"ErasureCodeMyClay: j->get<0>(): "<<(j->get<0>())<<", j->get<1>(): "<<(j->get<1>())<<dendl;
        for (int m = 0; m < (int)j->get<1>() && !error;
          ...
          dout(0)<<"ErasureCodeMyClay: handle_sub_read() FRAGMENTED store->read() of 1"<<dendl;
            ...
            r = store->read(
              ...
              bl00, j->get<2>());
            for (auto && k : subchunks_ind) {
              bl_tmp.substr_of(bl00, (k.first)*subchunk_size, (k.second)*subchunk_size);
              bl.claim_append(bl_tmp);
            }
          }
          dout(0)<<"ErasureCodeMyClay: handle_sub_read() FRAGMENTED read done"<<dendl;
        }
      }
