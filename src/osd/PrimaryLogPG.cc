...
struct FillInVerifyExtent : public Context {
  ...
  FillInVerifyExtent(ceph_le64 *r, int32_t *rv, bufferlist *blp,
		     ...
		     OSDService *osd, hobject_t soid, uint32_t flags, PGBackend *pgarg = nullptr) :
    r(r), rval(rv), outdatap(blp), maybe_crc(mc),
    size(size), osd(osd), soid(soid), flags(flags), pgbackend(pgarg) {}
  void finish(int len) override {
    ...
    *rval = 0;
    ...
    if (ecbackend != nullptr) {
      ...
      vector<int> order = ec_impl->get_myclay_order();
      if (!order.empty()) {
        ...
        char* stripe_buf = outdatap->c_str();
        for (int i=0; i<stripe_sz; i+=chunk_sz) {
          memcpy(tmp_buf, stripe_buf+i, chunk_sz);
          // order=[2,1,4,3] -> [1,2,3,4]
          for (int j=0; j<sc_count; j++) {
            int z=order[j];
            memcpy(stripe_buf+i+z*subchunk_sz, tmp_buf+j*subchunk_sz, subchunk_sz);
          }
        }
      }
    }
    osd->clog->error() << "ErasureCodeMyClay: full-object read done";
      ...
      make_pair(
        ...
        make_pair(&osd_op.outdata,
		  new FillInVerifyExtent(&op.extent.length, &osd_op.rval,
					 ...
					 osd, soid, op.flags, get_pgbackend()))));
