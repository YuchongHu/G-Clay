...
#include "ErasureCodePluginMyClay.h"
...
int ErasureCodePluginMyClay::factory(const std::string &directory,
				   ceph::ErasureCodeProfile &profile,
				   ceph::ErasureCodeInterfaceRef *erasure_code,
				   std::ostream *ss) {
  auto interface = std::make_unique<ErasureCodeMyClay>(directory);
...
int __erasure_code_init(char *plugin_name, char *directory)
{
  ...
  return instance.add(plugin_name, new ErasureCodePluginMyClay());
}
